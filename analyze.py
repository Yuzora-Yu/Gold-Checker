...
def analyze():
    ...
    try:
        print(f"--- Phase 1: Fetching Gold Data ---")
    ...
    try:
        ...
        print(f"Price: {latest_price}, Sync: {is_synced}, CISD_S: {cisd_sup}")
    except Exception as e:
    ...
    try:
        print("--- Phase 3: Scoring & Signal Detection ---")
        # 既存スコア計算
        ...
        # AI判定ロジックの拡張
        if is_golden:
            status, reason = "✨ GOLDEN SIGN", f"短期・長期の買い圧同調。CISDサポート ${cisd_sup:.1f}。"
        elif is_death:
            status, reason = "💀 DEATH SIGN", f"短期・長期の売り圧同調。CISDレジスタンス ${cisd_res:.1f}。"
        elif is_synced:
            status, reason = "✨期待値最大化ポイント", f"5m/30mトレンド同期中。{'上昇' if trend_5m==1 else '下落'}方向への加速に注目。"
        elif final_score_1h > 30:
            status, reason = "押し目買い", f"短期RSI {latest_rsi:.1f}。長期{trend_4h}トレンド内。"
        elif final_score_1h < -30:
            status, reason = "戻り売り", f"短期乖離 {latest_dev:.1f}%。長期{trend_4h}トレンド内。"
        else:
            status, reason = "静観", f"長期は{trend_4h}中。明確な同期または壁への到達待ち。"
    ...
    try:
        ...
        result = {
            ...
            "vacuum_zones": vacuum_zones[:3], # 上位3件
            "is_synced": is_synced,
